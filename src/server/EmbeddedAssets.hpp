...
--radius:12px;--radius-sm:8px;--radius-md:12px;--radius-lg:16px;
...
.app-main-col{flex:1;display:flex;flex-direction:column;min-width:0;min-height:0;overflow:hidden}
...
.sb-foot-meta{margin-top:.35rem;padding-top:.5rem!important;border-top:1px solid var(--border)}
.sb-foot-ws{font-size:.6875rem;font-family:var(--mono);color:var(--text-muted);display:flex;align-items:center;gap:.3rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.sb-foot-build{font-size:.6875rem;font-family:var(--mono);color:var(--text-muted);opacity:.8}
...
/* Ask User blocks */
.ask-user-block{margin:.75em 0;border-radius:var(--radius-sm);background:rgba(251,191,36,0.06);border:1px solid rgba(251,191,36,0.25);overflow:hidden;animation:askPulse 2s ease-in-out infinite}
@keyframes askPulse{0%,100%{border-color:rgba(251,191,36,0.25)}50%{border-color:rgba(251,191,36,0.5)}}
.ask-user-block:has(.ask-user-answered){animation:none;border-color:rgba(74,222,128,0.25);background:rgba(74,222,128,0.04)}
.ask-user-header{display:flex;align-items:center;gap:.5rem;padding:.625rem .75rem;font-size:.8125rem;font-weight:600;color:#fbbf24;border-bottom:1px solid rgba(251,191,36,0.12)}
.ask-user-block:has(.ask-user-answered) .ask-user-header{color:#4ade80}
.ask-user-question{padding:.625rem .75rem;font-size:.9375rem;color:var(--text-primary);line-height:1.5}
.ask-user-input-row{display:flex;gap:.5rem;padding:.5rem .75rem .625rem;align-items:center}
.ask-user-input{flex:1;background:var(--bg-body);border:1px solid var(--border);border-radius:var(--radius-sm);padding:.5rem .75rem;color:var(--text-primary);font-size:.875rem;font-family:var(--font);outline:none;transition:border-color .15s}
.ask-user-input:focus{border-color:var(--accent)}
.ask-user-btn{background:linear-gradient(135deg,#fbbf24,#f59e0b);color:#000;border:none;border-radius:var(--radius-sm);padding:.5rem 1rem;font-size:.8125rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:.35rem;transition:opacity .15s;white-space:nowrap}
.ask-user-btn:hover{opacity:.85}
.ask-user-btn:disabled{opacity:.5;cursor:default}
.ask-user-answered{padding:.5rem .75rem;font-size:.8125rem;color:#4ade80;display:flex;align-items:center;gap:.35rem}
...
.asset-thumb .asset-check{position:absolute;top:2px;right:2px;width:14px;height:14px;border-radius:50%;background:var(--accent);display:none;align-items:center;justify-content:center})html" R"html(
...
.notes-input{flex:1;background:var(--bg-elevated);border:1px solid var(--border);border-radius:6px;color:var(--text-primary);padding:.25rem .5rem;font-size:.8125rem;outline:none;font-family:var(--font)})html" R"html(
...
.deploy-section-title{font-size:.75rem;font-weight:600;text-transform:uppercase;letter-spacing:.04em;color:var(--text-muted);margin-bottom:.75rem})html" R"html(
...
/* @mention picker */
.mention-popup{position:absolute;bottom:100%;left:0;right:0;max-height:240px;overflow-y:auto;background:var(--bg-elevated);border:1px solid var(--border);border-radius:var(--radius-sm);margin-bottom:4px;display:none;z-index:50;box-shadow:0 -4px 16px rgba(0,0,0,0.3)}
...
.mention-item-desc{font-size:.6875rem;color:var(--text-muted);font-family:var(--mono);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:55%}
.mention-item.mention-hint{cursor:pointer;color:var(--text-muted);border-top:1px dashed var(--border);font-style:italic}
.mention-item.mention-hint:hover{background:rgba(124,58,237,0.06);color:var(--accent-pale)}
/* Sidebar-foot chat-target chip */
.sb-chat-target{padding:.2rem 0}
.chat-target-chip{display:flex;align-items:center;gap:.375rem;padding:.3rem .5rem;border-radius:var(--radius-sm);border:1px solid var(--border);background:var(--bg-elevated);font-size:.75rem;line-height:1.2;overflow:hidden}
.chat-target-chip.local{border-color:rgba(74,222,128,.25);background:rgba(74,222,128,.06)}
.chat-target-chip.remote{border-color:rgba(124,58,237,.45);background:rgba(124,58,237,.12);box-shadow:0 0 0 1px rgba(124,58,237,.25) inset}
.chat-target-dot{width:7px;height:7px;border-radius:50%;flex-shrink:0}
.chat-target-dot.online{background:#4ade80;box-shadow:0 0 6px rgba(74,222,128,.6)}
.chat-target-dot.remote{background:#a855f7;box-shadow:0 0 6px rgba(168,85,247,.6)}
.chat-target-dot.warning{background:#fbbf24}
.chat-target-dot.offline{background:#f87171}
.chat-target-arrow{color:#c4b5fd;line-height:0;margin:0 -.125rem}
.chat-target-label{color:var(--text-secondary);font-weight:600;flex-shrink:0}
.chat-target-chip.remote .chat-target-label{color:#c4b5fd}
.chat-target-meta{color:var(--text-muted);font-family:var(--mono);font-size:.6875rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0;flex:1}
.chat-target-clear{margin-left:auto;background:none;border:none;color:var(--text-muted);cursor:pointer;padding:2px;line-height:0;border-radius:3px;flex-shrink:0}
.chat-target-clear:hover{color:#f87171;background:rgba(248,113,113,.12)}
/* chat-input accent when targeting a remote node */
.chat-input.chat-input-remote{box-shadow:inset 0 2px 0 0 rgba(124,58,237,.55)}
.chat-input.chat-input-remote textarea{border-color:rgba(124,58,237,.45)}
.md-mention{display:inline-block;max-width:100%;padding:0 .28em;margin:0 .05em;border-radius:4px;font-weight:600;font-size:.92em;color:#c4b5fd;background:rgba(124,58,237,.18);border:1px solid rgba(124,58,237,.35);vertical-align:baseline}
.nodes-panel-head-actions{display:flex;align-items:center;gap:.125rem}
...
.remote-ui-bar .node-label{font-size:.875rem})html" R"html(
.remote-ui-frame{flex:1;border:none;width:100%;background:var(--bg-body)}
/* Nodes toggle (floating) hidden — use toolbar button */
.nodes-toggle{display:none}
.toolbar-nodes-btn{display:flex}

/* Desktop: dock nodes column in layout; collapse when not .open */
...
.nodes-panel{
  ...
  overflow:hidden;
  pointer-events:none;
  transition:flex-basis .22s ease,width .22s ease,min-width .22s ease,opacity .18s ease;
}
.nodes-panel.open{
  flex:0 0 240px;
  width:240px;
...
.mermaid-block .mermaid-render{position:relative;height:350px;overflow:hidden}
.mermaid-block svg,.sys-mermaid-wrap svg{position:absolute;left:0;top:0;overflow:visible;max-width:none}
.mermaid-error{color:#f87171;font-size:.8125rem;padding:.5rem}
.nodeLabel{white-space:normal!important;word-wrap:break-word!important;overflow-wrap:break-word!important;max-width:350px!important;display:inline-block!important;text-align:center!important;line-height:1.35!important}
.nodeLabel p{margin:0!important}
.node foreignObject{overflow:visible!important}
.node foreignObject>div,.node foreignObject>body,.node foreignObject>section{overflow:visible!important}
.edgeLabel .nodeLabel{max-width:160px!important}
.cluster-label .nodeLabel{max-width:none!important}
.pz-controls{position:absolute;top:8px;right:8px;display:flex;flex-direction:column;gap:4px;z-index:5}
.pz-btn{width:32px;height:32px;border-radius:8px;border:1px solid var(--border);background:rgba(12,12,24,0.85);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);color:var(--text-secondary);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .15s;padding:0;line-height:0}
.pz-btn svg{display:block;flex-shrink:0}
.pz-btn:hover{background:rgba(124,58,237,0.2);color:var(--accent-pale);border-color:var(--accent)}
.pz-btn:active{transform:scale(0.9)}
...
.apps-page-wrap .app-drawer{grid-template-columns:repeat(auto-fill,minmax(112px,1fr));gap:1rem 1.25rem;padding:.75rem 0;max-width:none;width:100%}
@media(min-width:900px){
.apps-page-wrap .app-drawer{grid-template-columns:repeat(auto-fill,minmax(132px,1fr));gap:1.25rem 1.5rem}
.apps-page-wrap .app-drawer-icon{width:76px;height:76px;border-radius:18px}
.apps-page-wrap .app-drawer-label{max-width:124px;font-size:.75rem}
}
...
.app-ctx-item{display:flex;align-items:center;gap:10px;padding:9px 14px;border-radius:8px;font-size:.8125rem;color:var(--text-primary);cursor:pointer;transition:background .1s})html" R"html(
...
.icon-picker-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:.5rem;padding:.5rem 1.25rem 1rem;max-height:280px;overflow-y:auto}
.icon-picker-item{width:72px;height:72px;border-radius:12px;overflow:hidden;cursor:pointer;border:2px solid transparent;transition:border-color .15s,transform .1s;background:var(--bg-surface)}
.icon-picker-item:hover{border-color:var(--accent);transform:scale(1.06)}
.icon-picker-item img{width:100%;height:100%;object-fit:cover;display:block}
...
/* ── System Page ────────────────────────────────────── */
.sys-wrap{max-width:1200px;margin:0 auto;padding:1.5rem 1rem;overflow-y:auto;height:100%}
.sys-header{margin-bottom:1.5rem}
.sys-header h2{font-size:1.25rem;font-weight:700;color:var(--text-primary);display:flex;align-items:center;gap:.5rem;margin-bottom:.25rem}
.sys-sub{font-size:.8125rem;color:var(--text-muted);line-height:1.5}
.sys-grid{display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1.25rem}
.sys-panel{background:var(--bg-elevated);border:1px solid var(--border);border-radius:var(--radius);display:flex;flex-direction:column;overflow:hidden}
.sys-panel-head{display:flex;align-items:center;justify-content:space-between;padding:.75rem 1rem;border-bottom:1px solid var(--border);font-size:.8125rem;font-weight:600;color:var(--text-secondary);gap:.5rem;flex-wrap:wrap}
.sys-panel-head span{display:flex;align-items:center;gap:.35rem}
.sys-panel-actions{display:flex;gap:.375rem;flex-wrap:wrap}
.sys-textarea{flex:1;width:100%;min-height:280px;resize:vertical;background:var(--bg-body);border:none;color:var(--text-primary);font-family:var(--mono);font-size:.8125rem;line-height:1.6;padding:.75rem 1rem;outline:none;box-sizing:border-box}
.sys-textarea:focus{background:rgba(124,58,237,0.02)}
.sys-prompt-meta{display:flex;justify-content:space-between;padding:.375rem .75rem;font-size:.6875rem;color:var(--text-muted);border-top:1px solid var(--border)}
.sys-mermaid-wrap{flex:1 0 420px;padding:0;overflow:hidden;position:relative}
.sys-mermaid-wrap .mermaid-render{width:100%;height:100%;position:relative;overflow:hidden}
.sys-flow-empty{text-align:center;color:var(--text-muted);font-size:.8125rem;padding:2rem}
.sys-flow-empty p{margin-top:.5rem}
.sys-section{background:var(--bg-elevated);border:1px solid var(--border);border-radius:var(--radius);padding:1rem;margin-bottom:1rem}
.sys-section-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:.75rem;flex-wrap:wrap;gap:.5rem}
.sys-section-head h3{font-size:.9375rem;font-weight:600;color:var(--text-primary);display:flex;align-items:center;gap:.375rem;margin:0}
.sys-btn{background:var(--bg-surface);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text-secondary);font-size:.75rem;padding:.375rem .625rem;cursor:pointer;display:inline-flex;align-items:center;gap:.25rem;transition:all .15s;font-weight:500;white-space:nowrap}
.sys-btn:hover{background:rgba(255,255,255,0.05);color:var(--text-primary);border-color:rgba(255,255,255,0.12)}
.sys-btn-primary{background:rgba(124,58,237,0.15);border-color:rgba(124,58,237,0.3);color:var(--accent-pale)}
.sys-btn-primary:hover{background:rgba(124,58,237,0.25)}
.sys-btn-danger{background:rgba(220,38,38,0.1);border-color:rgba(248,113,113,0.2);color:#f87171}
.sys-btn-danger:hover{background:rgba(220,38,38,0.2)}
.sys-btn-sm{font-size:.6875rem;padding:.3rem .5rem}

/* Flow builder */
.sys-fb-nodes{display:flex;flex-direction:column;gap:.5rem;margin-bottom:.75rem}
.sys-fb-hint{font-size:.8125rem;color:var(--text-muted);padding:.5rem 0}
.sys-fb-node{display:flex;gap:.5rem;align-items:center}
.sys-fb-input{flex:1;background:var(--bg-body);border:1px solid var(--border);border-radius:var(--radius-sm);padding:.375rem .625rem;color:var(--text-primary);font-size:.8125rem;font-family:var(--font);outline:none;min-width:0}
.sys-fb-input:focus{border-color:var(--accent)}
.sys-fb-select{background:var(--bg-body);border:1px solid var(--border);border-radius:var(--radius-sm);padding:.375rem .5rem;color:var(--text-primary);font-size:.8125rem;font-family:var(--font);outline:none;cursor:pointer;min-width:90px}
.sys-fb-del{background:none;border:none;color:var(--text-muted);cursor:pointer;padding:.25rem;border-radius:4px;transition:color .15s}
.sys-fb-del:hover{color:#f87171}
.sys-fb-edges{border-top:1px solid var(--border);padding-top:.75rem}
.sys-fb-edge-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:.5rem;font-size:.8125rem;font-weight:600;color:var(--text-secondary)}
.sys-fb-edge{display:flex;gap:.375rem;align-items:center;margin-bottom:.375rem;flex-wrap:wrap}
.sys-fb-arrow{color:var(--accent-pale);flex-shrink:0}
.sys-fb-elabel{max-width:120px}

/* Blueprint cards */
.sys-blueprints{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:.75rem}
.sys-bp-empty{text-align:center;color:var(--text-muted);font-size:.8125rem;padding:1.5rem;grid-column:1/-1}
.sys-bp-card{background:var(--bg-body);border:1px solid var(--border);border-radius:var(--radius);padding:.875rem 1rem;transition:border-color .15s,transform .1s}
.sys-bp-card:hover{border-color:var(--accent);transform:translateY(-1px)}
.sys-bp-card-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:.25rem}
.sys-bp-card-head strong{font-size:.875rem;color:var(--text-primary)}
.sys-bp-badge{font-size:.625rem;background:rgba(124,58,237,0.2);color:var(--accent-pale);padding:.125rem .375rem;border-radius:4px;font-weight:600}
.sys-bp-card-desc{font-size:.75rem;color:var(--text-muted);margin-bottom:.5rem;line-height:1.4;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical}
.sys-bp-card-meta{font-size:.6875rem;color:var(--text-muted);margin-bottom:.5rem}
.sys-bp-card-actions{display:flex;gap:.375rem;flex-wrap:wrap}

/* Flow builder model select */
.sys-fb-model{min-width:140px;max-width:180px;font-size:.6875rem;color:var(--accent-pale)}

/* Agent Builder AI */
.ab-section{border-color:rgba(124,58,237,0.2);background:linear-gradient(135deg,var(--bg-elevated) 0%,rgba(124,58,237,0.03) 100%)}
.ab-badge{font-size:.6875rem;background:rgba(124,58,237,0.15);color:var(--accent-pale);padding:.2rem .5rem;border-radius:6px;font-weight:500}
.ab-form{display:flex;flex-direction:column;gap:.75rem}
.ab-desc-input{width:100%;resize:vertical;background:var(--bg-body);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text-primary);font-family:var(--font);font-size:.8125rem;line-height:1.6;padding:.75rem;outline:none;box-sizing:border-box;transition:border-color .15s}
.ab-desc-input:focus{border-color:var(--accent)}
.ab-desc-input::placeholder{color:var(--text-muted);opacity:.7}
.ab-config{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.5rem}
.ab-config-section{display:flex;gap:.375rem;flex-wrap:wrap}
.ab-generate-btn{min-width:180px;justify-content:center;font-size:.8125rem;padding:.5rem 1rem}
.ab-generate-btn.disabled{opacity:.5;cursor:wait}
.spinner-sm{width:14px;height:14px;border:2px solid rgba(255,255,255,.2);border-top-color:var(--accent-pale);border-radius:50%;animation:spin .6s linear infinite;display:inline-block}
@keyframes spin{to{transform:rotate(360deg)}}

.ab-tool-picker,.ab-model-picker{background:var(--bg-body);border:1px solid var(--border);border-radius:var(--radius-sm);padding:.75rem;animation:fadeSlideIn .2s ease}
@keyframes fadeSlideIn{from{opacity:0;transform:translateY(-4px)}to{opacity:1;transform:translateY(0)}}
.ab-picker-head{font-size:.75rem;color:var(--text-muted);margin-bottom:.625rem;font-weight:500}
.ab-tool-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:.75rem}
.ab-tool-cat{background:rgba(255,255,255,0.015);border:1px solid rgba(255,255,255,0.04);border-radius:8px;padding:.5rem}
.ab-tool-cat-head{font-size:.6875rem;font-weight:600;color:var(--accent-pale);text-transform:uppercase;letter-spacing:.5px;margin-bottom:.375rem;display:flex;justify-content:space-between;align-items:center}
.ab-cat-toggle{font-size:.5625rem;background:rgba(124,58,237,0.1);border:1px solid rgba(124,58,237,0.2);color:var(--accent-pale);border-radius:4px;padding:1px 6px;cursor:pointer;text-transform:uppercase;letter-spacing:.5px}
.ab-cat-toggle:hover{background:rgba(124,58,237,0.2)}
.ab-tool-item{display:flex;align-items:center;gap:.375rem;font-size:.75rem;color:var(--text-secondary);padding:.2rem .25rem;border-radius:4px;cursor:pointer;transition:background .1s}
.ab-tool-item:hover{background:rgba(255,255,255,0.03)}
.ab-tool-item.selected{color:var(--accent-pale)}
.ab-tool-item input{accent-color:var(--accent);width:14px;height:14px}
.ab-tool-item code{font-size:.6875rem;background:rgba(255,255,255,0.04);padding:1px 4px;border-radius:3px}

.ab-model-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:.5rem}
.ab-model-row{display:flex;align-items:center;gap:.5rem}
.ab-model-label{font-size:.75rem;color:var(--text-secondary);min-width:120px;font-weight:500}
.ab-model-select{flex:1;background:var(--bg-surface);border:1px solid var(--border);border-radius:var(--radius-sm);padding:.375rem .5rem;color:var(--text-primary);font-size:.75rem;font-family:var(--mono);outline:none;cursor:pointer}
.ab-model-select:focus{border-color:var(--accent)}

.ab-result{background:rgba(124,58,237,0.04);border:1px solid rgba(124,58,237,0.15);border-radius:var(--radius-sm);padding:.875rem;animation:fadeSlideIn .25s ease}
.ab-result-head{margin-bottom:.5rem}
.ab-result-head strong{font-size:.9375rem;color:var(--text-primary)}
.ab-result-desc{display:block;font-size:.75rem;color:var(--text-muted);margin-top:.125rem}
.ab-result-label{font-size:.6875rem;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:.5px;margin-right:.375rem}
.ab-result-tools{margin-bottom:.5rem;line-height:2}
.ab-tool-tag{font-size:.625rem;background:rgba(99,102,241,0.15);color:#a5b4fc;padding:2px 6px;border-radius:4px;margin-right:2px}
.ab-result-models{margin-bottom:.625rem}
.ab-model-tag{display:inline-flex;align-items:center;gap:4px;font-size:.6875rem;background:rgba(124,58,237,0.08);border:1px solid rgba(124,58,237,0.12);padding:3px 8px;border-radius:6px;margin:2px 4px 2px 0;color:var(--text-secondary)}
.ab-model-tag span{color:var(--text-muted)}
.ab-model-tag code{font-size:.625rem;color:var(--accent-pale)}
.ab-result-actions{display:flex;gap:.375rem;flex-wrap:wrap;padding-top:.5rem;border-top:1px solid rgba(255,255,255,0.04)}

@media(max-width:768px){
.sys-grid{grid-template-columns:1fr}
.sys-textarea{min-height:200px}
.sys-mermaid-wrap{flex-basis:300px}
.mermaid-block .mermaid-render{height:250px}
.sys-blueprints{grid-template-columns:1fr}
.sys-fb-edge{flex-direction:column;align-items:stretch}
.sys-fb-edge .sys-fb-select{width:100%}
.sys-fb-arrow{display:none}
.sys-fb-elabel{max-width:100%}
.sys-panel-head{flex-direction:column;align-items:flex-start;gap:.5rem}
.ab-tool-grid{grid-template-columns:1fr}
.ab-model-grid{grid-template-columns:1fr}
.ab-config{flex-direction:column;align-items:stretch}
.ab-generate-btn{width:100%}
.sys-fb-node{flex-wrap:wrap}
.sys-fb-model{min-width:100%;max-width:100%}
}
@media(max-width:480px){
.sys-wrap{padding:1rem .5rem}
.sys-header h2{font-size:1.05rem}
.sys-section{padding:.75rem}
}
...
.dash-sub{font-size:.875rem;color:var(--text-muted);margin-bottom:1.5rem})html" R"html(
...
var el=document.createElement('div');)html" R"html(
...
else{for(var ci=0;ci<userTools.length;ci++)body+=renderToolItem(userTools[ci],builtins.length+ci);})html" R"html(
...
api('/api/tools/'+encodeURIComponent(name),{method:'DELETE'}))html" R"html(
.then(function(r){return r.json();}).then(function(d){
...
actions+='<button title="Delete" onclick="deleteAsset(\''+esc(a.id)+'\')" style="background:rgba(0,0,0,.7);border:none;color:#f87171;font-size:.6rem;padding:2px 4px;border-radius:3px;cursor:pointer">'+ic('trash',9)+'</button>';)html" R"html(
...
{r:'/knowledge',ic:'book',l:'Knowledge Base'},)html" R"html(
...
'<label style="display:flex;align-items:center;gap:.5rem;padding:.5rem .75rem;font-size:.8125rem;cursor:pointer;color:var(--text-secondary)">'+)html" R"html(
...
'<button class="note-toggle '+(n.enabled?'on':'off')+'" onclick="toggleNote('+i+')"></button>'+)html" R"html(
...
'<button class="pz-btn" onclick="pzZoom(this,0.7)" title="Zoom out">'+ic('minus',14)+'</button>'+)html" R"html(
...
var lp=aMsg.parts.length?aMsg.parts[aMsg.parts.length-1]:null;)html" R"html(
...
else if(l.indexOf('@@')===0)cls='diff-hunk';)html" R"html(
...
h+='<button class="node-action-btn" onclick="toggleNodesPanel()" title="Hide nodes">'+ic('xic',14)+'</button></span></div>';)html" R"html(
...
toolbar.classList.remove('mobile-active');)html" R"html(
...
if(activeTab===path){activeTab=openTabs.length?openTabs[Math.min(idx,openTabs.length-1)].path:'';})html" R"html(
render();setupIdeAfterRender();
};
...
var gy=pad+chartH*g/4;
var gv=Math.round(maxVal*(1-g/4));
gridLines+='<line x1="'+pad+'" y1="'+gy+'" x2="'+(w-pad)+'" y2="'+gy+'" stroke="rgba(255,255,255,0.04)" stroke-width="1"/>';)html" R"html(
gridLines+='<text x="'+(pad-8)+'" y="'+(gy+4)+'" fill="rgba(255,255,255,0.3)" font-size="10" text-anchor="end">'+fmtNum(gv)+'</text>';
}
...
if(sysState.mermaidSrc){)html" R"html(
...
avaConfirm('Reset the system prompt to the shipped default? Your current prompt will be overwritten.',function(){)html" R"html(
...
api('/api/system/blueprints',{method:'POST',body:{name:name,description:desc,prompt:prompt,nodes:sysState.nodes,edges:sysState.edges,mermaid_src:sysState.mermaidSrc||'',tools:[],model_assignments:{}}}).then(function(bp){)html" R"html(
...
'</div></div>'+
'<div class="set-row"><span class="set-label">Port</span><span class="set-value">'+((S.status&&S.status.port)?S.status.port:'--')+'</span></div></div>'+)html" R"html(
...
'</span></div>'+
'<button class="btn-sm" onclick="saveRelaySettings()" style="margin-top:.5rem">'+ic('save',12)+' Save Relay Settings</button>';)html" R"html(
} else if(role==='server'){
...
'<input id="kbSearchInput" type="text" placeholder="Search articles..." value="'+esc(kbState.search||'')+'" style="flex:1;background:var(--bg-elevated);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text-primary);padding:.5rem .75rem;font-size:.875rem;font-family:var(--font);outline:none" onkeydown="if(event.key===\'Enter\')doKbSearch()">'+)html" R"html(
...
h+='<div class="icon-picker-grid">';)html" R"html(
...
var fmt=function(n){if(typeof n!=='number'||!isFinite(n))return '\u2014';return n.toFixed(2);};)html" R"html(
...
'<div class="config-tags">'+tags+'</div></div></div>';)html" R"html(
}
...
if(!d.master_key_configured){)html" R"html(
